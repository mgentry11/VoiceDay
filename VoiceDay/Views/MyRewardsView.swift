...
struct MyRewardsView: View {
    ...
    private var pointsCard: some View {
        VStack(spacing: 16) {
            ...
            // Quick stats
            HStack(spacing: 24) {
                StatItem(label: "Today", value: "+\(rewardsService.streakData.tasksCompletedToday * 10)")
                StatItem(label: "This Week", value: "+\(rewardsService.streakData.weeklyTotal * 10)")
                StatItem(label: "Total", value: "\(rewardsService.totalPointsEarned)")
            }
        }
        .padding()
        .background(
