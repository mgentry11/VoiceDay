...
class TaskCoach: ObservableObject {
    ...
    func startCoaching(for task: VoiceDayTask) {
    ...
    private func determineQuestions(for task: VoiceDayTask) -> [QuestionType] {
    ...
    func quickQuestion(for task: VoiceDayTask) -> QuestionType {
        let titleLower = task.title.lowercased()
