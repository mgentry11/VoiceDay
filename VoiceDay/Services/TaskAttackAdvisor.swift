...
class TaskAttackAdvisor: ObservableObject {
    ...
    func analyzeTask(_ task: VoiceDayTask) -> TaskAnalysis {
    ...
    private func evaluateEnergyMatch(task: VoiceDayTask, duration: DurationEstimate) -> EnergyMatch {
        let energy = energyService.currentEnergy
    ...
    private func evaluateTiming(task: VoiceDayTask) -> TimingRecommendation {
        let energy = energyService.currentEnergy
    ...
    private func shouldSuggestBreakdown(task: VoiceDayTask, duration: DurationEstimate) -> Bool {
    ...
    private func generateBreakdownSuggestions(for task: VoiceDayTask) -> [String] {
        let titleLower = task.title.lowercased()
    ...
    private func generateTinyFirstStep(for task: VoiceDayTask) -> String {
        let titleLower = task.title.lowercased()
    ...
    func recommendNextTask(from tasks: [VoiceDayTask]) -> (task: VoiceDayTask, reason: String)? {
        ...
        // Score each task
        var scoredTasks: [(VoiceDayTask, Int, String)] = []
...
extension TaskAttackAdvisor {
    ...
    func recordAttempt(task: VoiceDayTask, strategy: AttackStrategy, completed: Bool) {
