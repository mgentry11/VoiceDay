...
struct VoiceDayTask: Identifiable, Codable {
    let id: UUID
    var title: String
    var dueDate: Date?
    ...
    mutating func addSubtask(_ subtask: VoiceDayTask) {
        if subtasks == nil {
            subtasks = []
        }
...
extension VoiceDayTask {

    /// Create subtasks from a list of step strings
    static func subtasksFrom(steps: [String], parentPriority: ItemPriority = .medium) -> [VoiceDayTask] {
        steps.map { step in
            VoiceDayTask(
                title: step,
